...
跑版本檢查時（Step 3 之後），順帶看一眼 `references/46-core-update-timeline.md` 頭部的「資料截至」日期：

- 距今超過 3 個月 → 提醒用戶：「另外，Google 更新時間表的資料到 [日期] 為止，如果之後要診斷流量下跌，最新更新記得對照官方 dashboard（status.search.google.com）」
- 該檔本身的過期防護規則在使用當下也會觸發，這裡只是更新檢查時的順帶提醒
