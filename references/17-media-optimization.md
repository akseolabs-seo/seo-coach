...
### AI 生成圖片的隱藏浮水印（認知，非規避教學）
如果用戶大量用 AI 產圖發文，要知道一件事：主流 AI 圖片會帶隱藏浮水印。Google 的 nano banana 模組自動帶 SynthID；GPT Image 2 從 2026 年五月起也全面加入。我自己有一批用 GPT Image 2 產圖的新站，因此被 Google 大規模移除（K 掉）。（as-of 2026-07 觀察）

網路上確實有移除這類浮水印（含 SynthID）的工具存在，但那是**規避偵測的灰帽手法**。教練的立場：解釋這個偵測機制、讓用戶理解「為什麼 AI 產圖的站容易被標記」，而不是教規避。正道是——圖片與內容要有真實價值：用真實照片、實測截圖、原創圖表，而不是靠躲浮水印過關。單靠 AI 大量產圖鋪站，本來就踩在品質風險線上。
